...
class Score:
    ...
    def score(colors_to_population: dict, filter_enabled: bool = False) -> Hct:
        ...
        scored_hct = []
        accent_hct = []
        for hct, population in zip(colors_hct, colors_to_population.values()):
            ...
            individual_proportion = population / population_sum
            tone_floor = max(0.0, min(1.0, (hct.tone - 12.0) / 48.0))
            tone_ceiling = max(0.0, min(1.0, (92.0 - hct.tone) / 32.0))
            tone_score = tone_floor * tone_ceiling
            salience_score = (hct.chroma**1.25) * (individual_proportion**0.3) * tone_score * 0.8

            score = proportion_score + chroma_score + salience_score
            scored_hct.append({"hct": hct, "score": score})

            if hct.chroma >= 32 and 18 <= hct.tone <= 76 and individual_proportion >= 0.0008:
                accent_score = (hct.chroma**1.55) * (individual_proportion**0.18) * tone_score
                accent_hct.append({"hct": hct, "score": accent_score})

        scored_hct.sort(key=lambda x: x["score"], reverse=True)
        accent_hct.sort(key=lambda x: x["score"], reverse=True)

        if accent_hct and accent_hct[0]["score"] >= 45:
            return DislikeAnalyzer.fix_if_disliked(accent_hct[0]["hct"])
