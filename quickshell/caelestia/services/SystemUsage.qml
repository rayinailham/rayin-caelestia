...
Singleton {
    ...
    // Network speed properties (bytes/sec)
    property real netDown: 0
    property real netUp: 0
    property real lastRxBytes: 0
    property real lastTxBytes: 0
    property bool netInitialized: false

    function formatSpeed(bytesPerSec: real): var {
        const kib = 1024;
        const mib = 1024 * 1024;
        const gib = 1024 * 1024 * 1024;

        if (bytesPerSec >= gib)
            return { value: (bytesPerSec / gib).toFixed(1), unit: "GB/s" };
        if (bytesPerSec >= mib)
            return { value: (bytesPerSec / mib).toFixed(1), unit: "MB/s" };
        if (bytesPerSec >= kib)
            return { value: (bytesPerSec / kib).toFixed(0), unit: "KB/s" };
    ...
    FileView {
        ...
        onLoaded: {
            ...
            if (root.netInitialized) {
                const interval = GlobalConfig.dashboard.resourceUpdateInterval / 1000;
                root.netDown = Math.max(0, (totalRx - root.lastRxBytes) / interval);
                root.netUp = Math.max(0, (totalTx - root.lastTxBytes) / interval);
            }
            root.lastRxBytes = totalRx;
            root.lastTxBytes = totalTx;
            root.netInitialized = true;
        }
    }
