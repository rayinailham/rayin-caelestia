...
Singleton {
    ...
    function translateDispatch(request: string): string {
        ...
        if (request.includes("hl.dsp") || request.includes("hl.dispatch") || request.includes("(") || request.includes("{")) {
            return request;
        }
        ...
        if (cmd === "workspace") {
            return `hl.dsp.focus({ workspace = "${argStr}" })`;
        } else if (cmd === "movetoworkspace") {
            const match = argStr.match(/^([^,]+),\s*address:(0x[a-fA-F0-9]+)$/);
            if (match) {
                return `hl.dsp.window.move({ window = "address:${match[2]}", workspace = "${match[1]}", follow = true })`;
            } else {
                return `hl.dsp.window.move({ workspace = "${argStr}", follow = true })`;
            }
        } else if (cmd === "togglespecialworkspace") {
            const name = argStr || "special";
            return `hl.dsp.workspace.toggle_special("${name}")`;
        } else if (cmd === "togglefloating") {
            if (argStr.startsWith("address:")) {
                return `hl.dsp.window.float({ window = "${argStr}" })`;
            } else {
                return "hl.dsp.window.float()";
            }
        } else if (cmd === "pin") {
            if (argStr.startsWith("address:")) {
                return `hl.dsp.window.pin({ window = "${argStr}" })`;
            } else {
                return "hl.dsp.window.pin()";
            }
        } else if (cmd === "killwindow") {
            if (argStr.startsWith("address:")) {
                return `hl.dsp.window.kill({ window = "${argStr}" })`;
            } else {
                return "hl.dsp.window.kill()";
            }
        } else if (cmd === "submap") {
            return `hl.dsp.submap("${argStr}")`;
        }
    ...
    function dispatch(request: string): void {
        Hyprland.dispatch(translateDispatch(request));
    }
