...
Item {
    ...
    readonly property FileDialog facePicker: FileDialog {
        ...
        onAccepted: path => {
            ...
            const faceGif = `${Paths.home}/.face.gif`;
            let success;
            if (isGif) {
                success = CUtils.copyFile(Qt.resolvedUrl(path), Qt.resolvedUrl(faceGif))
                    && CUtils.copyFile(Qt.resolvedUrl(path), Qt.resolvedUrl(face));
            } else {
                CUtils.deleteFile(Qt.resolvedUrl(faceGif));
                success = CUtils.copyFile(Qt.resolvedUrl(path), Qt.resolvedUrl(face));
            }
            if (success)
